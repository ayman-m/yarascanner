...
| `RULE-010` | ErrorLogger.close() — Windows file-handle release before cleanup | 1 | supporting | not_run | — | — |
...
| `RULE-053` | Cache pruning by file count and total bytes (LRU) | 1 | supporting | not_run | — | — |
| `RULE-054` | Orphaned cache temp sweep with a 1-hour age gate | 1 | low | not_run | — | — |
| `RULE-055` | compile_source / compile_seconds telemetry | 3 | supporting | not_run | — | — |
| `RULE-056` | rule_cache survives end-of-run host cleanup | 1 | core | not_run | — | — |
...
| `TRAV-025` | Producer backpressure: files are blocked on, never dropped | 1 | core | not_run | — | — |
| `TRAV-026` | Per-directory heartbeat call during the walk (rate-limited to YARA_HEARTBEAT_SECS) | 1 | supporting | not_run | — | — |
...
| `TRAV-034` | Windows drive-letter exclusion list (present but permanently empty)  <sub>windows</sub> | 1 | low | not_run | — | — |
...
| `TRAV-049` | DORMANT: real-path de-duplication across junctions (track_real_paths is hard-wired off) | 1 | low | not_run | — | — |
| `TRAV-050` | File-existence gate at dequeue time | 3 | supporting | not_run | — | — |
| `TRAV-051` | Read-permission gate with per-file permission diagnostics (unbounded, unthrottled) | 1 | supporting | not_run | — | — |
...
| `TRAV-057` | files_skipped on the wire: every scan-lifecycle row carries the skip count | 1 | core | not_run | — | — |
...
| `TRAV-064` | Scan status transitions are effectively unobservable (and their uploader is never called) | 1 | low | not_run | — | — |
...
| `PERF-001` | CPU governor policy selector (headroom / budget / none) | 1 | core | not_run | — | — |
| `PERF-002` | Headroom policy — always leave N% of the host free | 1 | core | not_run | — | — |
| `PERF-003` | Budget policy — fixed cap on the scanner's share | 1 | core | not_run | — | — |
| `PERF-004` | CPU floor — the anti-stall guarantee (headroom policy only) | 1 | supporting | not_run | — | — |
| `PERF-005` | Process-CPU normalisation to a whole-machine share | 1 | supporting | not_run | — | — |
| `PERF-006` | Sleep-ratio controller (proportional gain and runaway clamp) | 1 | supporting | not_run | — | — |
| `PERF-007` | Per-file proportional pacing (the actuator) | 1 | core | not_run | — | — |
| `PERF-008` | Governor sampling rate limit | 1 | supporting | not_run | — | — |
| `PERF-009` | Governor fail-open on unreadable CPU | 1 | supporting | not_run | — | — |
| `PERF-010` | Governor telemetry: change-triggered plus heartbeat emission | 1 | supporting | not_run | — | — |
| `PERF-011` | psutil CPU sampler priming | 1 | supporting | not_run | — | — |
| `PERF-012` | THROTTLE_CONFIG startup header in the performance log | 1 | supporting | not_run | — | — |
| `PERF-013` | Below-normal process priority and I/O priority demotion | 1 | core | not_run | — | — |
| `PERF-014` | DEAD CODE: idle-tier ("os") priority branch | 1 | low | not_run | — | — |
| `PERF-015` | CPU affinity capture (Cortex agent core pinning) | 1 | supporting | not_run | — | — |
| `PERF-016` | host_cores recorded outside the affinity try-block | 1 | supporting | not_run | — | — |
| `PERF-017` | Worker thread count and the auto (cores // 2) mode | 1 | core | not_run | — | — |
| `PERF-018` | Worker pool startup and naming | 1 | supporting | not_run | — | — |
| `PERF-019` | Bounded scan queue (the memory ceiling for file discovery) | 1 | supporting | not_run | — | — |
| `PERF-020` | Producer backpressure — block, never drop | 1 | core | not_run | — | — |
| `PERF-021` | Queue-saturation event counter with 1-in-25 log sampling | 1 | low | not_run | — | — |
| `PERF-022` | Governor sampling from the blocked producer | 1 | supporting | not_run | — | — |
...
| `PERF-027` | Per-worker throughput logging every 100 files — and why its Error Rate is structurally 0.0% | 1 | low | not_run | — | — |
| `PERF-028` | Per-worker timing ring buffer capped at 100 samples — and the end-of-run summary that reports its length as a file count | 1 | supporting | not_run | — | — |
| `PERF-029` | Progress heartbeat thread (whole-scan progress telemetry) | 1 | core | not_run | — | — |
| `PERF-030` | Progress snapshot holds lock_counts across psutil calls | 1 | supporting | not_run | — | — |
| `PERF-031` | Progress sampler reuses the primed psutil handle | 1 | supporting | not_run | — | — |
...
| `PERF-033` | ETA and completion-time estimation | 1 | supporting | not_run | — | — |
| `PERF-034` | Scan-lifecycle heartbeat thread (dataset 'running' rows) | 1 | core | not_run | — | — |
| `PERF-035` | Heartbeat gate is lock-protected against duplicate rows | 1 | supporting | not_run | — | — |
| `PERF-036` | Paused-seconds accounting on every lifecycle row | 1 | supporting | not_run | — | — |
...
| `PERF-040` | StatisticsManager background performance sampler | 1 | supporting | not_run | — | — |
| `PERF-041` | Performance history ring buffer and peak/average metrics | 1 | low | not_run | — | — |
| `PERF-042` | Sampled performance-detail logging (1 in 6 snapshots) | 1 | low | not_run | — | — |
| `PERF-043` | Snapshot enrichment with live scanner counters | 1 | low | not_run | — | — |
| `PERF-044` | SystemResourceMonitor thread (host-level resource sampling) | 1 | supporting | not_run | — | — |
| `PERF-045` | Resource alert thresholds (CPU / memory / disk) | 1 | supporting | not_run | — | — |
| `PERF-046` | Resource and alert history ring buffers | 1 | supporting | not_run | — | — |
| `PERF-047` | Resource trend classification | 1 | low | not_run | — | — |
| `PERF-048` | Resource monitor stopped AFTER worker join, not at discovery end | 1 | supporting | not_run | — | — |
| `PERF-049` | File-descriptor sampling every 1000 CLEAN scanned files  <sub>linux, darwin</sub> | 1 | low | not_run | — | — |
| `PERF-050` | Startup file-descriptor limit probe  <sub>linux, darwin</sub> | 1 | supporting | not_run | — | — |
| `PERF-051` | FD monitoring flag plumbing (two-name handoff)  <sub>linux, darwin</sub> | 1 | supporting | not_run | — | — |
...
| `PERF-075` | DEAD CODE: _get_scanner_stats aggregate | 1 | low | not_run | — | — |
| `PERF-076` | DEAD CODE: periodic scan-status upload | 1 | low | not_run | — | — |
...
| `PERF-079` | End-of-run performance summary lines | 1 | core | not_run | — | — |
| `PERF-080` | Both psutil monitors are OFF by default — every performance figure in the final report is structurally zero | 1 | core | not_run | — | — |
| `PERF-081` | Per-file permission denials accumulate in an unbounded list that nothing ever reads | 1 | supporting | not_run | — | — |
| `PERF-082` | Unthrottled 'Permission denied' system-log line — one record per unreadable file | 1 | supporting | not_run | — | — |
| `PERF-083` | Mislabelled resource-monitor telemetry: monitoring_duration_minutes is host uptime | 1 | low | not_run | — | — |
| `PERF-084` | Per-tick 'Network: X MB' is the whole host's traffic since boot, not the scanner's uploads | 1 | low | not_run | — | — |
...
| `PERF-086` | Governor final state persisted as a structured cpu_governor block in the run summary | 1 | core | not_run | — | — |
| `PERF-087` | Per-worker throughput reports are time-gated, not file-count-gated | 1 | supporting | not_run | — | — |
| `PERF-088` | Governor sampling-cadence counters (`samples_taken`, `secs_since_last_sample`) | 1 | supporting | not_run | — | — |
| `PERF-089` | FD sampling runs once per file PROCESSED, before every early return | 2 | supporting | not_run | — | — |
| `STOR-001` | Scanner working directory root (scanner_dir) and its platform defaults | 1 | supporting | not_run | — | — |
| `STOR-002` | Fixed subdirectory layout under scanner_dir (logs, control, alert, evidence, failed_rules — plus rule_cache) | 1 | supporting | not_run | — | — |
| `STOR-003` | run_id — microsecond timestamp that names every per-run file | 1 | supporting | not_run | — | — |
| `STOR-004` | Six per-category structured log files (alerts / statistics / errors / performance / uploads / system) | 1 | supporting | not_run | — | — |
...
| `STOR-008` | Orphaned scan_summary *.tmp sweep at scan start | 1 | low | not_run | — | — |
| `STOR-009` | Log retention — keep only the last N scans' logs and summaries | 1 | supporting | not_run | — | — |
| `STOR-010` | Current run is force-protected from retention | 1 | supporting | not_run | — | — |
| `STOR-011` | Log-file deletion failures are tolerated, not fatal | 1 | low | not_run | — | — |
...
| `STOR-014` | Progress-heartbeat writes to statistics AND performance logs on a fixed cadence for the whole scan | 1 | supporting | not_run | — | — |
| `STOR-015` | config.output_log (scanner_<run_id>.log) — DEAD as a log file, but load-bearing as a path | 1 | low | not_run | — | — |
| `STOR-016` | initial_cleanup wipes the previous run's alert/ and evidence/ directories wholesale | 1 | supporting | not_run | — | — |
...
| `STOR-021` | Evidence ZIP creation and naming | 1 | core | not_run | — | — |
...
| `STOR-024` | Metadata-only evidence ZIP is the default (collect_files=false) | 1 | supporting | not_run | — | — |
| `STOR-025` | Evidence ZIP bundles only alert/*.txt — .alert files are excluded by design | 1 | low | not_run | — | — |
...
| `STOR-028` | Runtime-generated cleanup script (cleanup_script.sh / .bat) in scanner_dir root | 1 | supporting | not_run | — | — |
| `STOR-029` | Alert rotation: .txt → .alert, executed by the scheduled task, not by the scan | 1 | supporting | not_run | — | — |
| `STOR-030` | Windows scheduled task 'CleanupScript' registered as SYSTEM  <sub>windows</sub> | 1 | supporting | not_run | — | — |
| `STOR-031` | Linux (and any non-Windows/non-Darwin) systemd unit /etc/systemd/system/yara-cleanup.service — written, ENABLED, never removed  <sub>linux</sub> | 1 | supporting | not_run | — | — |
...
| `STOR-033` | Cleanup scheduling is gated on at least one alert .txt existing | 1 | supporting | not_run | — | — |
...
| `STOR-036` | control/running.json — atomically-refreshed liveness marker | 1 | supporting | not_run | — | — |
...
| `STOR-039` | rule_cache/ — compiled-ruleset disk cache (XDR-only; the XSIAM twin has none) | 1 | core | not_run | — | — |
...
| `STOR-042` | Rule-cache atomic save with PID+random temp naming | 1 | supporting | not_run | — | — |
| `STOR-043` | Rule-cache orphan .tmp sweep, age-gated at 1 hour | 1 | low | not_run | — | — |
...
| `STOR-046` | rule_cache is deliberately exempt from HostCleanup | 1 | supporting | not_run | — | — |
...
| `STOR-050` | HostCleanup — opt-in end-of-run removal of this run's on-host working files | 1 | core | not_run | — | — |
| `STOR-051` | HostCleanup KEEP tiers (nothing / summary / evidence) | 1 | supporting | not_run | — | — |
| `STOR-052` | HostCleanup refuses to delete unless the summary JSON durably exists | 1 | core | not_run | — | — |
| `STOR-053` | HostCleanup on_delivery gate — refuses when there is no delivery channel to verify | 1 | core | not_run | — | — |
| `STOR-054` | HostCleanup runs only on outcome=='completed' | 3 | core | not_run | — | — |
| `STOR-055` | HostCleanup closes log FileHandlers before deleting (Windows WinError 32) | 1 | supporting | not_run | — | — |
| `STOR-056` | HostCleanup recreates alert/evidence/failed_rules empty after wiping | 1 | supporting | not_run | — | — |
| `STOR-057` | HostCleanup identifies this run's logs the same way retention does | 1 | supporting | not_run | — | — |
...
| `STOR-063` | Resource-monitor sampling histories are ring-buffered (memory, not disk) | 1 | supporting | not_run | — | — |
...
| `STOR-066` | Runtime fingerprint (embedded Python, platform, yara binding version) written at the head of yara_processing_<run_id>.log before any rule work | 1 | supporting | not_run | — | — |
...
| `STOR-068` | StatisticsManager bypasses LogManager and writes raw, multi-line blocks into statistics_/performance_<run_id>.log | 1 | low | not_run | — | — |
| `STOR-069` | Logging counters under-report by construction, and yara_processing_<run_id>.log is missing from log_files_created | 1 | low | not_run | — | — |
| `STOR-070` | Evidence ZIP is produced on every completed scan, including zero-match runs — its existence proves nothing | 1 | core | not_run | — | — |
| `STOR-071` | Alert-directory byte ceiling degrades detail, never counts (`YARA_ALERT_DIR_MAX_MB`) | 2 | core | not_run | — | — |
| `STOR-072` | The root diagnostics handler is closed before host cleanup | 1 | supporting | not_run | — | — |
...
| `DELI-035` | Scans-dataset heartbeat cadence | 1 | supporting | not_run | — | — |
| `DELI-036` | Independent heartbeat thread (decoupled from the directory walker) | 1 | supporting | not_run | — | — |
...
| `DELI-064` | CPU governor telemetry heartbeat (CPU_GOVERNOR lines) | 1 | supporting | not_run | — | — |
...
| `DELI-070` | Log retention across runs (delivery diagnostics window) | 1 | supporting | not_run | — | — |
...
| `LIFE-001` | Action Center scan entry point (main) — only 3 operator inputs | 1 | core | not_run | — | — |
...
| `LIFE-021` | running.json liveness marker — write, heartbeat refresh, removal | 1 | supporting | not_run | — | — |
| `LIFE-022` | CANCEL_DRAIN_DEADLINE_SECS — dead constant | 3 | low | not_run | — | — |
| `LIFE-023` | Scan phase ordering in scan_system | 1 | supporting | not_run | — | — |
| `LIFE-024` | Scan-lifecycle rows in the yara_scanner_scans dataset | 1 | core | not_run | — | — |
| `LIFE-025` | Terminal lifecycle row emitted after workers drain but before uploaders stop | 1 | core | not_run | — | — |
| `LIFE-026` | Heartbeat lifecycle row and its independent thread | 1 | supporting | not_run | — | — |
| `LIFE-027` | Progress-heartbeat thread spanning the whole scan | 1 | supporting | not_run | — | — |
| `LIFE-028` | Shutdown sequence in _perform_enhanced_cleanup | 1 | supporting | not_run | — | — |
| `LIFE-029` | Worker-thread join timeout is non-fatal | 1 | supporting | not_run | — | — |
...
| `LIFE-048` | Root-logger INFO records land in diagnostics_<run_id>.log | 1 | supporting | not_run | — | — |
...
| `LIFE-052` | Dead/unused lifecycle constants and config attributes | 1 | low | not_run | — | — |
| `LIFE-053` | Unreachable branches: ScanConfig mode=cancel and _discover_all_targets | 3 | low | not_run | — | — |
| `LIFE-054` | logs/scanner_<run_id>.log — declared, cleaned, self-excluded, never written | 1 | low | not_run | — | — |
...
| `LIFE-057` | Log retention across runs (keep last N scans) plus orphan-temp sweep | 1 | core | not_run | — | — |
| `LIFE-058` | initial_cleanup — previous run's alert/evidence wiped at scan start | 1 | supporting | not_run | — | — |
| `LIFE-059` | schedule_final_cleanup gating (critical errors / error ratio / no alerts) | 1 | supporting | not_run | — | — |
| `LIFE-060` | Cleanup script generated from the real alert dir (path-drift fix) | 1 | supporting | not_run | — | — |
| `LIFE-061` | Platform-specific cleanup scheduling and its non-fatal failure modes | 1 | supporting | not_run | — | — |
| `LIFE-062` | End-of-run host cleanup — opt-in deletion of this run's working files | 1 | core | not_run | — | — |
| `LIFE-063` | Host cleanup refuses to run without a durable summary, and when there is no delivery channel | 2 | core | not_run | — | — |
| `LIFE-064` | Log handlers closed BEFORE host cleanup because Windows refuses to delete open files | 1 | supporting | not_run | — | — |
| `LIFE-065` | stop_logging idempotence and the final logging summary | 1 | supporting | not_run | — | — |
| `LIFE-066` | Monitoring lifecycle and its stop-once guards | 1 | supporting | not_run | — | — |
| `LIFE-067` | File-descriptor monitoring setup block (POSIX only, off by default)  <sub>linux, darwin</sub> | 1 | supporting | not_run | — | — |
...
| `LIFE-073` | Remaining-thread join in the successful path | 1 | low | not_run | — | — |
| `LIFE-074` | Terminal "completed" status emission is best-effort and last | 2 | core | not_run | — | — |
| `LIFE-075` | Run identity: run_id, scan_id and their propagation | 1 | core | not_run | — | — |
| `LIFE-076` | Scanner version self-identification | 1 | low | not_run | — | — |
| `LIFE-077` | "Scan configuration established" — resolved target list logged under a non-canonical scan_id | 3 | low | not_run | — | — |
| `LIFE-078` | "All monitoring systems activated" — the run's monitoring and delivery switch record (and why performance_metrics is all zeros) | 1 | supporting | not_run | — | — |
...
| `LIFE-081` | init_data initialisation disclosure record — emitted twice, includes the tenant API URL | 1 | low | not_run | — | — |
| `LIFE-082` | A failed category logger silently falls back to the root logger | 1 | supporting | not_run | — | — |
