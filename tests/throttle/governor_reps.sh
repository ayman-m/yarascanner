...
set -u
TARGET="${TARGET:-/usr}"
OUT=~/governor_reps; rm -rf "$OUT"; mkdir -p "$OUT"
...
for r in 1 2 3; do
  ...
  run idle_headroom_r$r "create_alerts=false,write_dataset=false,cpu_guarantee=headroom"

  echo "=== 2. idle, ungoverned (throughput ceiling) ==="
  run idle_none_r$r "create_alerts=false,write_dataset=false,cpu_guarantee=none"
  ...
  LP=$!; sleep 15
  run loaded_headroom_r$r "create_alerts=false,write_dataset=false,cpu_guarantee=headroom"
  ...
  LP=$!; sleep 15
  run loaded_budget_r$r "create_alerts=false,write_dataset=false,cpu_guarantee=budget,cpu_budget_pct=20"
  kill -TERM $LP 2>/dev/null; wait $LP 2>/dev/null

done

echo "=== GOVERNOR REPEATS DONE ==="
