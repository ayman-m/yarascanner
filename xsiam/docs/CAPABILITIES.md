...
`_iter_hit_fields` used to accept two shapes: a live yara Match object, and a plain dict whose `strings` were `(offset, id, hex-text)` triples rehydrated with `bytes.fromhex`, falling back to a utf-8 encode of the raw value. The dict shape implied a match CACHE — something that serialised findings and replayed them later. No such cache exists in this scanner, and the arm has been deleted.

Kept in this reference rather than dropped, so a reader who finds the shape in an older copy, in the XDR edition, or in a downstream fork learns what happened to it.

**Why it was unreachable**, enumerated rather than assumed: three call sites (the `rules_matched` comprehension, the `_write_alerts` loop, and the `total_string_matches` sum) all iterate `matches`; `matches` has exactly ONE binding, `matches = self.rules.match(...)`, which returns `yara.Match` objects and never dicts; `_write_alerts` receives `matches` as a parameter but has a single caller passing that same local; and no module outside the scanner imports `_iter_hit_fields`.

**Why it was removed rather than kept for safety** — it was not safe. The decode fallback was `hx.encode("utf-8", errors="ignore")` on anything `bytes.fromhex` rejected, so a non-hex string produced WRONG BYTES silently instead of raising, and every offset and matched-data field downstream would have been quietly corrupt.

The deletion also surfaced a testing gap worth recording: six alert offset-cap tests were building dict-shaped hits, so the cap that prevents a 220 MB alert file had only ever been unit-tested through the branch production never runs. The behaviour was correct — a live storm-file scan showed `Matched Strings (showing 50 of 6000)` — but those tests were not what confirmed it. Their fixture now builds a Match-shaped hit carrying bytes rather than hex text, which is the difference that let the two paths drift apart unnoticed.

- **Control:** Removed — `_iter_hit_fields` now accepts the live Match shape only. — default `-`
- **Observe:** Passing a dict now raises `AttributeError` instead of being silently mis-decoded. Pinned by `tests/test_hit_field_extraction.py`, which also fixes the modern (`identifier`/`instances`), legacy tuple, and condition-only shapes so the surviving contract cannot drift.
- **Source:** `_iter_hit_fields — single-shape after removal; tests/test_hit_field_extraction.py`
...
A FAILED probe is cached too: otherwise an unwritable /tmp costs one exception per scanned file, which is the worst case rather than the rare one. The probe runs under a lock so racing workers cannot double-probe. Windows and Linux answer from policy and never touch the disk.
...
- **Observe:** `samples_taken` and `secs_since_last_sample` in the `CPU governor \|` event's data payload (yara_scans_raw). These report the SAMPLING cadence, which is not the emission cadence the lines appear at — lines are gated by a change threshold plus a 30 s heartbeat. Measured live on xsoar the two differ by ~60x: 104 samples at 0.501 s spacing across a 74.8 s scan, against lines every 30 s. The first sample reports `null` rather than 0.0, so a cold start is not mistaken for two instantaneous readings, and a disabled governor stays at 0 so it cannot look busy.
...
### File-descriptor leak sampling, on every file processed  <sub>linux, darwin</sub>

**⚠ CONTROL GAP**
When FD monitoring is on, the scanner re-reads its own open-FD count every `fd_check_interval` files and warns on growth. `_maybe_sample_fds()` is called at the top of `scan_file()`, before every early return, so the cadence counts files PROCESSED rather than files that happened to survive to the end of the method.

It previously sat at the END of `scan_file`, after `return True, "Scanned and matched"` and after every skip bail-out, so it ran only on files that were scanned and did NOT match. On the Round 2 flood (8,003 files, 4,002 matching) over half the scan bypassed it; against a ruleset matching everything it never ran at all — FD monitoring went quiet exactly when the scanner held the most handles. The counter increment was also unlocked, so it raced across workers and silently dropped counts, making the effective interval longer than configured and impossible to reason about from the settings.

The counter now moves under `lock_counts`, and a healthy sample records `fd_samples_taken` and `last_fd_count` — previously only threshold breaches emitted anything, so "sampled 40 times, all fine" and "never sampled" left identical evidence. A failed psutil read returns without counting and never raises: a scan is worth more than its FD telemetry.
...
- **Observe:** `fd_samples_taken` and `last_fd_count` on the scanner after a run with `YARA_ENABLE_FD_MONITOR=true`. Samples must equal files/interval EVEN WHEN EVERY FILE MATCHES — verified live on xsoar with an all-matching ruleset: 500 files, 500 matches, 10 samples at interval 50. That configuration previously produced ZERO samples, because the check sat after `return True, "Scanned and matched"`.
...
### .txt -> .alert rotation, run synchronously by the scan on Linux

The script's only job is to rename every alert/*.txt to *.alert. This is what marks a run's alert texts as consumed/rotated so the NEXT run's _check_for_alerts (which tests only for .txt) and the next evidence ZIP (which packages only .txt) do not pick them up. The name "scheduled" describes the Windows path: on Linux the scan starts the unit itself and the rename has already happened by the time the result line prints. The gate still sees .txt, because _check_for_alerts runs before the unit is started — though note initial_cleanup wipes the whole alert directory at the next scan start anyway.

- **Control:** not configurable — default `rename *.txt -> *.alert in alert_dir`
- **Observe:** On Linux there is nothing to wait for. List alert/ immediately after main() returns: files are ALREADY <rule>.alert. _schedule_linux_cleanup ends with `systemctl start yara-cleanup.service` — a synchronous check=True call against a Type=oneshot unit whose ExecStart is the rename script — so the rotation completes inside the scan's own finalisation. Verified on a live endpoint: a scan writing lc_ok.txt was followed by `AFTER_SCAN1 alert: ['lc_ok.alert']` with no scheduler involved, and re-running cleanup_script.sh by hand renamed nothing because no .txt remained. On Windows the task is registered rather than started, so there the wait is real; `schtasks /query /tn CleanupScript /v` showing Last Result 0 confirms the rename ran (this path previously reported Last Result = 1 with the wrong hardcoded directory).
...
- **Observe:** `scanner_initialization` data.upload_enabled / data.telemetry_upload_enabled echo the two flags, and the delivery books separate them. Three runs are needed because neither switch can be isolated from one: both on -> match 8 uploaded, telemetry 9 requests; `UPLOAD_RESULTS=False` -> match 0, telemetry 0, with 8 findings still on disk in 1 local alert file; `UPLOAD_NON_MATCH_DATA=False` -> match 8 STILL uploaded, telemetry 0. The uploads log tracks it too (3,095 / 216 / 2,747 bytes).
