...
Writes ONE ROW PER (host, rule) into yara_scanner_summary_v<VER>_rules_<hash> - ONE dataset
per RULESET, holding every host scanned with it. Grouping is by the ruleset hash the scanner
leaves at the end of every scan_id, which is the only component all hosts in one Action
Center scan share; scan_id itself is "{hostname}_{run_id}_yara_{hash}", so keying targets on
it produced one dataset per host and consolidated nothing.

Re-running reconciles rather than appends: it compares the scan_ids the target already holds
against the ones the sources hold now. Unchanged hosts are left alone, a re-scanned host's
old rows are dropped and its new ones written. Running twice with nothing changed is a
verified no-op. Rows keep their own scan_id and hostname, so provenance survives grouping.

CAVEATS, stated rather than engineered around:
  - Two separate launches using an IDENTICAL ruleset share a hash. This is MEASURED, not
    hypothetical: the tenant's own audit log shows launches at 2026-08-20 06:29:52 and
    2026-08-21 04:24:41, both ruleset b451b4336910, both over the same three endpoints, 22h
    apart. When every host is re-scanned this is harmless - each host's matches dataset is
    overwritten, so the target reconciles wholesale to the newer launch. It only matters on
    a PARTIAL re-scan (below). Tell launches apart by scan_id or event_timestamp_ms.
  - A host NOT re-scanned stays in the group with its older scan_id. The dataset is the
    current state of every host on that ruleset, which is not always the same thing as one
    Action Center action.
  - A host that matched NOTHING contributes no rows and is invisible here. Absence from this
    dataset means "no findings", never "not scanned" - the scans lifecycle shard is where
    you confirm a host actually ran.
  - A host scanned with a DIFFERENT ruleset moves to that ruleset's dataset and its old rows
    are dropped from this one.

WHY NOT the Action Center group_action_id, which would be exact? It cannot be discovered.
Probed live: /actions/get_actions/, /actions/get_all_actions/, /actions/get_actions_status/
and /audits/actions/ all return HTTP 500, and get_action_status without an id returns 500
"group_action_id param is missing". The id would have to be typed in by hand from the console
for every run. The ruleset hash needs no input and works on data already on the tenant.
...
def summary_target_name(ver, scan_id):
    ...
    RETAINED FOR THE OLD PER-SCAN LAYOUT ONLY. Summarising now groups by ruleset - see
    summary_target_for_rules - because a per-scan target is a per-HOST target: scan_id is
    "{hostname}_{run_id}_yara_{rule_hash}" by construction, so keying on it produced exactly
    as many summary datasets as there were hosts, which is not a consolidation at all."""
...
def summary_target_for_rules(ver, rule_hash):
    """yara_scanner_summary_v<VER>_rules_<hash>. ONE dataset per ruleset, holding every host
    scanned with it - the consolidation the name always promised."""
    slug = re.sub(r"[^a-z0-9]+", "_", str(rule_hash or "unknown").lower()).strip("_") or "unknown"
    return "%s_%s_v%s_rules_%s" % (_PREFIX, _SUMMARY_KIND, ver, slug)
...
def _host_of_scan_id(scan_id):
    """The hostname a scan_id was minted from, or None. Splitting on "_" would break any
    hostname containing one, so the fixed-shape tail is matched and removed instead."""
    s = str(scan_id or "")
...
def _shard_host_of(hostname):
    """The `host` segment the scanner builds a dataset name from. Mirrors
    _dataset_shard_suffix in xdr_yara_scanner.py exactly - slugify, cap at 32, prefix a letter
    if needed, append six hex of the sha1 of the ORIGINAL hostname."""
    raw = str(hostname or "")
    if not raw:
        return None
...
def _supersedable(missing, target, existing, ver, log):
    """Of the scans the target holds but no source does, the ones genuinely superseded.

    `stale = held - observed` reads absence from the sources as proof the scanner overwrote
    the scan. That holds while the sources are there, and stops holding the moment something
    else removed them - which FULL consolidation now does, retiring a host dataset once its
    rows are verified in the ruleset target. Summary reads those same host datasets, so
    without this guard a summary pass after a full pass deletes the summary rows of every
    host full mode retired. Seen on the lab tenant: 9 retired hosts, still held in the
    summary target, all of them one run away from being dropped.

    A host's matches dataset name is STABLE - permanent, overwritten in place, never rotated
    - so it returns on that host's next scan. That separates the two cases:

      * host dataset PRESENT, scan absent from it -> the scanner overwrote it. Superseded.
      * host dataset ABSENT entirely              -> retired, or never created. Says nothing
                                                     about the scan. Keep the rows.
    """
    missing = set(missing)
    if not missing:
        return missing
    ...
    for sid in missing:
        ...
        if ds and ds in existing:
            ...
            kept.append(sid)
    if kept:
        log("  %d scan(s) held in %s have no source dataset at all - kept, since an absent "
...
def main():
    ...
    try:
        ...
        # "{hostname}_{run_id}_yara_{rule_hash}" by construction, so keying a target on it
        # produced one summary dataset PER HOST - the same dataset count we started with,
        # which is not a consolidation. The hash is the only component every host in one
        # Action Center scan shares. Gating stays per-scan: a host still mid-scan is kept
        # out of the group rather than dragging the whole dataset into a half-written state.
        groups = {}
        for sid in sorted(by_scan):
            ...
            for (host, rule), ts in sorted(pairs.items()):
                g["rows"].append({"scan_id": sid, "hostname": host, "rule": rule,
                                  "event_timestamp_ms": int(ts) if ts else (newest or now_ms)})
                g["hosts"].add(host)
            g["sids"].add(sid)
            g["why"].add("completed" if terminal
                         else "no lifecycle activity for %gh" % retention_hours)

        # ---- 3b. one dataset per ruleset, reconciled against what is already there -------
        for rh in sorted(groups):
            g = groups[rh]
            target = summary_target_for_rules(ver, rh)
            ...
            # run_id), so comparing scan_id sets tells us what changed without reading a
            # single row back.
            held = set()
            ...
            # when the scan_id set happened to match meant a target that had been partially
            # written, hand-edited, or left short by a failed batch stayed wrong forever,
            # because scan_id equality was taken as proof the CONTENT was equal too.
            ...
            try:
                ...
                removed = 0
                if refreshed:
                    # Same narrow deletion as `stale` below, against this automation's OWN
                    # output: clear the rows for the scans about to be rewritten. Without it a
                    # refresh would append a second copy of every (host, rule) pair, and the
                    # summary would silently double.
                    try:
                ...
                if stale:
                    ...
                    # output: rows for a scan the source no longer holds. Source shards are
                    # never touched. Serialised by the consolidation lock, which an executed
                    # pass already holds - remove_data is not concurrency-safe.
                    # Filter shape is COLUMN -> VALUE, OR'd across the list. The
                    # field/operator/value form this used to send is rejected by the
                    # endpoint with HTTP 500 ("'list' object has no attribute 'items'"),
                    # so reconciliation never removed a row. Probed live against the tenant.
                    reply = client.remove_lookup_data(
                        target, [{"scan_id": s} for s in stale])
                ...
                for i in range(0, len(add_rows), _WRITE_BATCH):
                    reply = client.add_lookup_data(target, add_rows[i:i + _WRITE_BATCH])
                    got = _added(reply)
                    if got <= 0:
                        ok = False
                        break
        ...
        # removed from a host matches shard or a scans shard. The host matches dataset is
        # permanent and is overwritten by the NEXT scan on that host; the scans shard is the
        # lifecycle record every future pass gates on.
        #
        # The one exception, deliberate and narrow: remove_lookup_data above, against this
        # automation's OWN summary target, for rows belonging to a scan the source no longer
        # holds. That is maintenance of its own output, not deletion of anyone's data - and
        # without it a re-run after a re-scan could only append, duplicating (host, rule)
        # pairs it has no way to undo.
