"""YaraReport - read-only inventory of this tenant's YARA lookup datasets.

Lists every yara_scanner_* lookup dataset with its kind, host and age in months, split into
current-schema, legacy and newer-schema buckets. It separates the two unsuffixed states that
need opposite advice: `frozen` (rotated siblings exist, so this is a pre-rotation leftover
that is no longer growing) and `not_rotated` (no siblings - rotation is off for that
deployment and the dataset will grow without bound). Per-scan consolidated targets are
reported as their own state and are never treated as either.

Writes nothing and deletes nothing, and issues a single API call, so it is safe to run at any
time - including from a poll loop, or alongside a consolidation or cleanup pass.

ARGUMENTS
  schema_version   The scanner's current lookup schema version (YARA_LOOKUP_SCHEMA_VER on the
                   endpoints). Must be a whole number. It decides which datasets count as
                   current, which as legacy and which as newer. Defaults to
                   DEFAULT_LOOKUP_SCHEMA_VERSION below.
"""
    ...
    for name in current:
        ...
        if info is None:
            datasets.append({"name": name,
                             "kind": "summary" if is_summary_dataset(name) else "",
                             "host": "", "month": "", "age_months": None,
                             "state": "summary" if is_summary_dataset(name) else "unrecognised"})
            continue
        if info["scan_target"]:
...
def main():
    args = demisto.args()
    try:
        # Always explicit, never inherited: the module global set_schema_version writes
        # outlives one execution in a long-running script container.
