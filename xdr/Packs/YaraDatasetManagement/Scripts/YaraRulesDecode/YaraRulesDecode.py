...
def decode_b64(text):
    """base64 -> (bytes, error_message). Never raises."""
    s = re.sub(r"\s+", "", str(text or ""))
    if not s:
        return None, "No base64 input given - pass b64, or entry_id of a file containing it."
    if _URLSAFE_RE.search(s):
        return None, ("Input looks like URL-safe base64 (it contains - or _). The scanner's "
                      "yarafile input is standard base64; re-encode with the standard "
                      "alphabet.")
    if len(s) % 4:
        s += "=" * (4 - len(s) % 4)
    if not _B64_ALPHABET_RE.match(s):
        return None, ("Input is not base64: it contains characters outside the base64 "
                      "alphabet. If this was pasted from a document, it may have picked up "
                      "quotes or line markers.")
    try:
        return base64.b64decode(s, validate=True), ""
    except Exception as ex:
        return None, "Input is not valid base64: %s" % str(ex)[:120]
...
def decode_rules(b64_text, expected_hash="", validate=True, max_bytes=DEFAULT_MAX_BYTES):
    ...
    out = {"ok": False, "errors": [], "rules": "", "rule_hash": "", "rule_names": [],
           "rule_count": 0, "size_bytes": 0, "expected_hash": str(expected_hash or "").strip(),
           "hash_matches": None}
    ...
    if not validate:
        ...
        text = _decode_bytes(raw)
        if text is None:
            out["errors"].append("Decoded bytes are not text in any supported encoding.")
            return out
        text = text.lstrip("\ufeff")
        out["rules"] = text
        out["size_bytes"] = len(text.encode("utf-8", "replace"))
...
def main():
    ...
    if result["ok"]:
        lines = ["Decoded %d bytes of YARA rules." % result["size_bytes"]]
        if result["rule_count"]:
            lines.append("%d rule(s): %s" % (result["rule_count"],
                                             ", ".join(result["rule_names"][:12])
                                             + (" ..." if result["rule_count"] > 12 else "")))
        lines.append("ruleset hash: `%s`" % result["rule_hash"])
    else:
        lines = ["Could NOT decode this payload into usable YARA rules.", ""]
        lines += ["  - %s" % e for e in result["errors"]]

    if result["hash_matches"] is True:
        lines.append("Hash MATCHES `%s` - this text is what that scan dispatched."
                     % result["expected_hash"])
    elif result["hash_matches"] is False:
        lines.append("Hash MISMATCH: this text hashes to `%s`, not the expected `%s`. The "
                     "payload is not the ruleset it was said to be."
                     % (result["rule_hash"], result["expected_hash"]))

    if result["rules"]:
        body = result["rules"]
        shown = body if len(body) <= 3000 else body[:3000] + "\n... (truncated)"
        lines += ["", "```", shown, "```"]

    results = [CommandResults(readable_output="\n".join(lines),
