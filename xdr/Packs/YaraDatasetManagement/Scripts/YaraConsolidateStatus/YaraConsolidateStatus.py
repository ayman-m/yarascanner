...
ARGUMENTS
  scan_id                Restrict the check to these scan_id(s), comma-separated. Omit to
                         check every finished scan.
  schema_version         The scanner's YARA_LOOKUP_SCHEMA_VER. Selects which shards are in
                         scope; defaults to DEFAULT_LOOKUP_SCHEMA_VERSION.
  retention_hours        A scan silent this long is treated as finished even without a
                         terminal lifecycle row. Default 24.
  quiet_secs             Override DEFAULT_QUIET_SECS for this run.

These four are the whole argument surface: they are what main() parses and what the .yml
declares. row_ceiling and abandoned_after_hours were listed here once and were never either
- they belong to check_consolidation_status, part of the RETIRED per-scan library inlined
below, which main() does not call. Passing them did nothing, silently. If you want the row
ceiling previewed, see the note on check_readiness: it does not model one today.
"""
...
# Merge gates. Only DEFAULT_QUIET_SECS is overridable per run from a script argument; the
# other two belong to the RETIRED per-scan path below and are settable from nothing.
