...
def main():
    ...
    max_scans = args.get("max_scans")
    kwargs = {"only_scan_ids": scan_ids, "dry_run": False,
              "max_scans": int(max_scans) if max_scans else DEFAULT_MAX_SCANS_PER_PASS}
    if quiet_secs:
    ...
    try:
        ...
        # Recorded BEFORE the merge, not only after. The platform kills a task that overruns
        # its timeout, and a kill runs no Python - so the terminal row below and the
        # except-branch "crashed" row are both unreachable in exactly the case worth
        # diagnosing. Measured on emea: a pass died at the 900s timeout and left no row at
        # all, so nothing in the tenant recorded that it had ever run. A "started" row with
        # no matching terminal row is what makes that state queryable.
        ...
        # Record the crash as a queryable row BEFORE return_error: return_error halts this
        # task, so the playbook's own "needs attention" flag is never reached from here. Only
        # possible if CoreApiClient() constructed - with placeholder credentials it did not.
    ...
    if result.get("lock_held_by_other_run"):
        lines = ["Skipped this pass — the consolidation lock is held by another "
                 "concurrent run."]
    else:
        lines = ["{} scan(s) consolidated".format(result["consolidated_count"])]
        if result.get("stopped_early"):
            lines.append("Pass was bounded so it would finish inside the task timeout - more "
                         "scans remain eligible. Run again to continue.")
    ...
    # Lock events exist ONLY in the library's log stream, never in the structured result: a
    # stale-lock takeover (which force-deletes another run's marker) and a failed release
    # (which parks every following pass until the marker goes stale).
    ...
    # it first so consolidated/deferred/failed never carry a prior call's entries.
