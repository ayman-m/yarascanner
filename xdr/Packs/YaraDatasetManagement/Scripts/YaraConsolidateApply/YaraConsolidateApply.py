        ...
        for rh in sorted(groups):
            ...
            if sources_complete:
                ...
                if held - observed.get(rh, set()):
                    ...
                    skipped.append("%s: stale-row removal SKIPPED - %s, so a scan missing from "
                                   "the sources is not evidence it was superseded"
                                   % (label, why_skipped))
...
def main():
    ...
    if execute:
        ...
        # the log exists to answer. It gets its own status.
